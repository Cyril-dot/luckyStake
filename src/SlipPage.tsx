...
import { api, fetchMatchDetail, formatKickoff, isLiveStatus, liveClock, normalizeMatches, parseKickoff, type MatchRow } from './api';
...
function normalizeSelection(raw: AnyRecord, index: number): SlipSelection {
  ...
  return {
    ...
    odds: numberValue(raw.submittedOdds, raw.odds, raw.odd, raw.price, raw.value),
    ...
    scoreHome: raw.scoreHome ?? raw.score_home ?? match.scoreHome,
    scoreAway: raw.scoreAway ?? raw.score_away ?? match.scoreAway,
  };
}
...
function TicketCard({ bet, matchUpdates, onDetails, onCashout, cashingOut }: { bet: SlipBet; matchUpdates: Record<string, MatchRow>; onDetails: () => void; onCashout: () => void; cashingOut: boolean }) {
  ...
  return <article className={`ls-ticket ${status === 'WON' ? 'ls-ticket-won' : status === 'LOST' ? 'ls-ticket-lost' : 'ls-ticket-open'}`} onClick={onDetails} role="button" tabIndex={0} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') onDetails(); }}>
    <div className="ls-ticket-head"><div><small>{bet.selections.length || 1} selection{bet.selections.length === 1 ? '' : 's'}</small><strong>{dateLabel(bet.placedAt)}</strong></div><span className={`ls-ticket-status ${status === 'WON' ? 'won' : status === 'LOST' ? 'lost' : ''}`}>{status}</span></div>
    <div className="ls-ticket-body">
      {(bet.selections.length ? bet.selections : [{ id: 'empty', matchId: '', homeTeam: 'Bet selection', awayTeam: '', market: 'Match result', selection: 'Awaiting selection', odds: bet.odds }]).map((selection, index) => {
        ...
        const live = isLiveStatus(matchStatus) || Boolean(scoreHome != null && scoreAway != null && kickoff && !Number.isNaN(parseKickoff(kickoff).getTime()) && Date.now() > parseKickoff(kickoff).getTime());
        return <div className="ls-ticket-leg" key={`${selection.id}-${index}`}><div className="ls-ticket-match"><span>{homeTeam} <em>vs</em> {awayTeam}</span><b>{selection.odds ? selection.odds.toFixed(2) : '—'}</b></div><div className="ls-ticket-meta">{live ? <><i className="ls-live-dot" /> <strong className="ls-live-label">LIVE {match ? liveClock(match) : ''}</strong>{scoreHome != null && scoreAway != null && <strong className="ls-live-score">{scoreHome} - {scoreAway}</strong>}</> : kickoff && <span>{formatKickoff(kickoff)}</span>}<span>· {selection.market}</span></div><div className="ls-ticket-pick"><span>{selection.selection}</span><strong>{selection.odds ? selection.odds.toFixed(2) : '—'}</strong></div></div>;
      })}
...
export default function SlipPage() {
  ...
  useEffect(() => {
    ...
    const refreshMatches = () => Promise.all([
      Promise.all(ids.map(async id => { try { return await fetchMatchDetail(id); } catch { return null; } })),
      api<unknown>('GET', '/api/public/football/matches/live').then(payload => normalizeMatches(payload, 'football')).catch(() => [] as MatchRow[]),
